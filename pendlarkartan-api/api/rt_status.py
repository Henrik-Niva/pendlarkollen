...
# Cache för alla operatörer
_STATUS_CACHE = {"ts": 0.0, "data": None}

# Cache per operatör
_STATUS_CACHE_BY_OP: dict[str, dict] = {}
...
def _check_operator(operator: str) -> dict:
    try:
        ...
        return {
            "operator": operator,
            "ok": False,
            "reason": f"Kunde inte läsa statisk GTFS för {operator}.",
            "debug": {"error": str(e)},
        }
    ...
    if not candidates:
        return {
            "operator": operator,
            "ok": False,
            "reason": f"Inga linjer hittades för {operator}.",
            "debug": {"candidates": []},
        }
...
# 🟢 EN OPERATÖR
# ===============================
@router.get("/realtime/status/{operator}")
def realtime_status_for_operator(operator: str):
    operator = validate_operator(operator)
    now = time.time()

    cached = _STATUS_CACHE_BY_OP.get(operator)

    if cached is not None and now - cached["ts"] < STATUS_TTL_SECONDS:
        return cached["data"]

    result = _check_operator(operator)

    _STATUS_CACHE_BY_OP[operator] = {
        "ts": now,
        "data": result,
    }
